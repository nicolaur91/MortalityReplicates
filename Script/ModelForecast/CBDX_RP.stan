data {
	...
	matrix[J,H] mort; // matrix of mortality to bx
...
parameters 
	...
	matrix<lower=-8,upper=0>[J,H] bx; // vector of bx
	...
	matrix[H,T] kappa_1_tilde; // governs std. normal parameterization of kappa_1
	matrix[H,T] kappa_2_tilde; // governs std. normal parameterization of kappa_2
...
transformed parameters 
	...
	for (t in 2:T)
		...
		k[,t] = c1+k[,t-1]-psy*(k[,t-1]-k_bar[t-1])+sigma[1]*kappa_1_tilde[,t];
		k2[,t] = c2+k2[,t-1]-psy*(k2[,t-1]-k2_bar[t-1])+sigma[2]*kappa_2_tilde[,t]*sqrt(1-square(rho))+rho*sigma[2]*(k[,t]-c1-k[,t-1]+psy*(k[,t-1]-k_bar[t-1]))/sigma[1];
...
model 
	...
	for (h in 1:H)
	{
		...
		for (t in 1:T) for (x in 1:J) 
			{
			//mu[pos] = e[pos]*(bx[x,h]+k[h,t]+(age[x]-mean(age))*k2[h,t]); //Predictor dynamics - likelihood
			mu[pos] = offset[pos]*(bx[x,h]+k[h,t]+(age[x]-mean(age))*k2[h,t]); //Predictor dynamics - likelihood
			pos += 1;
			}
	...
	for (h in 1:H)
	{
	target += std_normal_lpdf(kappa_1_tilde[h,]);
	target += std_normal_lpdf(kappa_2_tilde[h,]);
	}
